...
def main(args):
  ...
  with wandb.init(project='Doom Dreamer RL', config=config.__dict__):
    ...
    for episode in range(1, config.train_episodes+1):
      ...
      prev_action = torch.zeros(1, env.action_size, dtype=torch.float16, device=device)
      ...
      while not done:
        ...
        if train_steps % config.save_every_steps == 0:
          trainer.save_model(train_steps)
        ...
        trainer.replay_memory.add(observation, action.squeeze(0).detach().cpu().numpy(), reward, done)
...
if __name__ =="__main__":
  ...
  parser.add_argument('--seq_len', type=int, default=32, help='Sequence Length (chunk length)')
